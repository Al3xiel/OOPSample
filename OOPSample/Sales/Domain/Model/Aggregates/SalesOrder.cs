...
public class SalesOrder(int customerId)
{
    ...
    public void AddItem(int productId, int quantity, double unitPrice)
    {
        ...
        {
            throw new InvalidOperationException("Can not modify an order once payment is processed");
        }
        _items.Add(new SalesOrderItem(Id, productId, quantity, unitPrice));
    }
    public void Cancel()
    {
        if (Status == ESalesOrderStatus.Completed)
        {
            throw new InvalidOperationException("Can not cancel a completed order");
        }
